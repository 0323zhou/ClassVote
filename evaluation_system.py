...
def calculate_results():
    ...
    # 2. 统计团员互评 (票数)
    votes_df = pd.read_sql("SELECT candidate_uid as uid, COUNT(*) as vote_count FROM peer_votes GROUP BY candidate_uid",
                           conn)

    # 3. 统计组织评议 (班干票数)
    officer_votes_df = pd.read_sql(
        "SELECT candidate_uid as uid, COUNT(*) as officer_vote_count FROM officer_votes GROUP BY candidate_uid", conn)
    ...
    # 排名
    df = df.sort_values(by=['final_score', 'org_score', 'vote_count', 'self_score'],
...
def main():
    ...
    if user['role'] in ['student', 'officer']:
        ...
        with tabs[1]:
            ...
            if cur.fetchone()[0] > 0:
                ...
                # 排除自己
                candidates = pd.read_sql("SELECT uid, name FROM users WHERE role!='admin' AND uid != ?", conn,
                                         params=(user['uid'],))
                options = {row['uid']: f"{row['name']} ({row['uid']})" for i, row in candidates.iterrows()}
                selected = st.multiselect("候选人列表:", options.keys(), format_func=lambda x: options[x],
                                          key="peer_select")
        ...
        if user['role'] == 'officer':
            with tabs[2]:
                ...
                if cur.fetchone()[0] > 0:
                    ...
                    selected_off = st.multiselect("请慎重推选 10 人:", options_off.keys(),
                                                  format_func=lambda x: options_off[x], key="officer_select")

                    st.caption(f"已选: {len(selected_off)} / 10")
                    if st.button("提交组织评议"):
                        if len(selected_off) != 10:
                            st.error("规则限制：必须选择 10 人！")
                        else:
    ...
    # 2. 管理员界面
        ...
        st.download_button("下载CSV", df.to_csv().encode('utf-8-sig'), "result.csv")
